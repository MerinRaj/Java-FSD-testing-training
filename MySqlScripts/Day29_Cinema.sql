...
select f.name as film_name,max(s.id)as max_screenings,min(s.id) as min_screenings from screenings s
join films f on f.id=s.film_id
group by f.name;

select f.name as film_name,count(s.id)as no_of_screenings from films f
join screenings s on f.id=s.film_id
group by f.name;
